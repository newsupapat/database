...
router.data = (req, res) => {
  if (req.query.Get == "data") {
    statement.find({}, function(err, statement_data) {
      daily
        .find({ End_time: { $ne: null } }, "Start_time End_time")
        .exec(function(err, daily_data) {
          statement.find(
            {
              Date: {
                $gte: new Date(
              ...
              $or: [{ Code: "OT" }, { Code: "LATE" }]
            },
            "Date Code Sub_total",
            function(err, ot_time) {
              information
                .find({}, "_id s_Salary DOB")
                .sort({
                  _id: 1
                })
                .exec(function(err, salary) {
                  lasthist
                    .find({}, "Nuber_of_company_worked")
                    .exec(function(err, lastcom) {
                      information
                        .find({}, "Marital")
                        .exec(function(err, Marital) {
                          information
                            .find({}, "Nationality")
                            .exec(function(err, nation) {
                              education
                                .find({}, "EM_id GPAX")
                                .exec(function(err, gpax) {
                                  daily.find(
                                    {
                                      Date: {
                                        $gte: new Date(
